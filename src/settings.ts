...
export function renderProviderSettings(containerEl: HTMLElement, host: SettingsHost): () => void {
 ...
 const perform = async (work: (signal: AbortSignal) => Promise<void>) => {
  ...
  finally { operation = undefined; shared.busy = false; if (!disposed) notify(); else for (const listener of shared.listeners) listener(); }
 };
 ...
 const draw = () => {
  ...
  if (!models) containerEl.createEl("p", { text: "Agent runtime unavailable. Update the Obsidian desktop installer if its embedded Node runtime is unsupported." });
   ...
   new Setting(containerEl).setName("Provider").setDesc("Choose explicitly; no provider or billable model is selected automatically.").addDropdown(dropdown => {
   ...
   if (provider) {
    ...
    if (provider.auth.apiKey) {
     ...
     if (auth.login) setting.addButton(button => button.setButtonText("Connect / reconnect").setDisabled(disabled).onClick(() => authenticate("api_key")));
    ...
    if (provider.auth.oauth) new Setting(containerEl).setName(provider.auth.oauth.name).setDesc("Advertised by the provider; successful access depends on network, provider policy and your account entitlement.").addButton(button => button.setButtonText(provider.auth.oauth!.loginLabel ?? "Sign in / reconnect").setDisabled(disabled).onClick(() => authenticate("oauth")));
    new Setting(containerEl).setName("Connection").addButton(button => button.setButtonText("Disconnect").setDisabled(disabled).onClick(() => { void perform(async signal => {
     await models.logout(provider.id, { signal });
     shared.message = "Plugin-stored login removed. Ambient environment/profile authentication may still be available.";
     await host.connectionChanged();
    }); })).addButton(button => button.setButtonText("Refresh models").setDisabled(disabled).onClick(() => { void perform(async signal => { await refresh(provider.id, signal); await host.connectionChanged(); }); }));
