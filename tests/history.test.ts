...
describe("durable chat history", () => {
 it("serializes an in-flight save before deletion so a late write cannot resurrect a chat", async () => {
  let bytes = "";
  ...
  const deletion = store.delete("a"); gate.resolve(); await save; await deletion;
  expect(await new HistoryStore(adapter, "history.json").list()).toEqual([]);
  expect(bytes).not.toContain("Private note content");
 });
 it("does not report failed deletion as success or publish failed writes to readers", async () => {
  ...
  const store = new HistoryStore(adapter, "history.json"); await store.save(conversation("a")); fail = true;
  await expect(store.delete("a")).rejects.toThrow("history");
  await expect(store.save(conversation("b"))).rejects.toThrow("history");
  expect((await store.list()).map(c => c.id)).toEqual(["a"]);
  expect((await new HistoryStore(adapter, "history.json").list()).map(c => c.id)).toEqual(["a"]);
  fail = false; await store.delete("a"); expect(await store.list()).toEqual([]);
 });
