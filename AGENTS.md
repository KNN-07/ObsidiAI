...
TypeScript performs no emission; esbuild produces Node/CommonJS ES2022 output and lowers dynamic imports. Bundle application dependencies, leaving Obsidian/Electron, host CodeMirror/Lezer modules, and Node built-ins external. Preserve lazy undici initialization, lexical `fetch`/`globalThis.fetch` injection, and the undici build workaround—never assign the real host fetch. Google/Bedrock deliberately omit the explicit custom-fetch option.
